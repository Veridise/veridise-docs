# Running OrCa Through Command Line
...
## Setting Up OrCa Configuration

The main input to OrCa is a configuration JSON that gives OrCa all of the necessary information to run. To run the vault example, make sure the `src_path` is a relative path to the folder containing the source code to be tested (the `Vault` contract) and the `specs_path` is a path to a folder containing the desired specfication, in this case the following:

```
vars: Vault v
inv: reverted(v.deposit, v.closed)
```

A full writeup of the configuration options can be found in [the local configuration documentation](../developer_guide/local_configuration/configuration_options.md).

## Running OrCa

OrCa can be run with the following command from the root directory of the OrCa project:

```bash
python3 orca.py <PATH_TO_CONFIG>
```

### OrCa Output

OrCa will print out details of the current behavior of OrCa, including how long it has been running and the number of transactions it has found. If it finds a counterexample, it reports that as shown below for the `Vault` example:

```
Counter-example
Counterexample found in 1.611 seconds (6 transactions issued)!
--------------------------------------------------------------
vars: address addr0, address addr1
init: finished(vault_0 = Vault(), sender = addr0)

test: finished(vault_0.close(), sender = addr1) ;
      finished(vault_0.deposit(), sender = addr1 && value = 284867)
--------------------------------------------------------------
```
