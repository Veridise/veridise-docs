...
The commitment computation on line 8, however, does not properly downgrade the
...
1. Via the constraint to `commitment` and `operation` (which is generated by the `commitment <== operation + private_key` statement).
1. Via dataflow assignment to output signal `commitment` (also generated by the `commitment <== operation + private_key` statement).
